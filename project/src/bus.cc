    ...
    if (distance_remaining_ <= 0) {
        ...
        if (outgoing_route_->IsAtEnd()) {
            ...
            if (!incoming_route_->IsAtEnd()) {
                ...
                // if any passengers on or off, all distance to next stop
                // is left but, if we didn't handle any passengers here,
                ...
                if (passengers_handled != 0) {
                  ...
                  std::cout << "Skip Stop: " << next_stop_-> GetId()
                    << std::endl;
                }
        ...
        if (passengers_handled != 0) {
          ...
          std::cout << "Skip Stop: " << next_stop_-> GetId() << std::endl;
        }
...
void Bus::Update() {  // using common Update format
  Move();
  UpdateBusData();
  // Notify the bus's observer with the updated data
...
void Bus::UpdateBusData() {
    bus_data_.id = name_;
    int num_passengers = static_cast<int>(passengers_.size());

    Color c;
    // Get the correct route and early exit
    // set up the color for different route
    // set up the intensity based on the number of passengers
    Route * current_route = outgoing_route_;
    if (outgoing_route_->IsAtEnd()) {
    ...
    bus_data_.num_passengers = num_passengers;
