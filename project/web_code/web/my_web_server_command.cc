...
void AddListenerCommand::execute(MyWebServerSession* session, picojson::value& command, MyWebServerSessionState* state) {
    mySim->ClearListeners();  // clear listeners first
    std::cout << "starting AddListenerCommand::execute" << std::endl;
    std::string id = command.get<picojson::object>()["id"].get<std::string>();
    std::cout << id << std::endl;
    // add listener to the bus
    BusWebObserver* busWebObserver = new BusWebObserver(session);
    mySim->AddListenerBus(&id, busWebObserver);
}

AddListenerStopCommand::AddListenerStopCommand(VisualizationSimulator* sim) : mySim(sim) {}

void AddListenerStopCommand::execute(MyWebServerSession* session, picojson::value& command, MyWebServerSessionState* state) {
    mySim->ClearListeners();  // clear listeners first
    std::cout << "starting AddListenerStopCommand::execute" << std::endl;
    std::string id = command.get<picojson::object>()["id"].get<std::string>();
    std::cout << id << std::endl;
    // add listener to the stop
    StopWebObserver* stopWebObserver = new StopWebObserver(session);
    mySim->AddListenerStop(&id, stopWebObserver);
}
